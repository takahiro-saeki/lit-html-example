...
var Todo = function (_CustomElement2) {
  ...
  Todo.prototype.change = function change(newValue) {
    var newState = this.state.data.map(function (item) {
      if (item.id === newValue) {
    ...
    var filterData = this.filter(this.state.currentFilter);
  ...
  Todo.prototype.delete = function _delete(newValue) {
    var newState = this.state.data.filter(function (item) {
  ...
  Todo.prototype.filterState = function filterState(newValue) {
    var optimize = this.state.filter.map(function (item) {
      var param = {
        type: item.type,
  ...
  Todo.prototype.allCheckState = function allCheckState() {
    var optimize = this.state.data.map(function (item) {
  ...
  Todo.prototype.init = function init(newValue) {
    if (!this.read()) {
      var _optimize = this.state.filter.map(function (item) {
        var param = {
          type: item.type,
          isSelected: item.type === newValue ? true : false
        };
      ...
      var _filterData = this.filter(this.state.currentFilter);
      this.generate(_filterData, this.state.filter);
      return;
    }
...
var App = function App(data, filter) {
  return (0, _litHtml.html)(_templateObject, console.log('checking', { data: data, filter: filter }), (0, _Header2.default)('lit-todo', 'placeholder'), (0, _MainField2.default)(data, filter));
};
